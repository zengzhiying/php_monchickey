...
/**
 * 阴历阳历转换工具类
 * 阳历范围严格限制在闭区间:[1952.01.01 ~ 2038.01.19] 实际上是到2037.12.31 之后这19天大写年份不正确,但是其余的正确
 * 农历范围严格限制在闭区间:[1953年正月初一 ~ 2050年腊月廿九]
 */
class LunarCalendar1
{
    ...
    public function solarToLunar($date)
    {
        ...
        return array(
            ...
            $Lmonth,
            $Lday,
            $this->LYearName($Lyear),
            $this->LMonName($Lmonth),
            $this->LDayName($Lday),
        );
    ...
    //农历转公历(date：农历日期； type：是否闰月0不是闰月 1表示闰月 比如2017-6-2 1表示的的闰六月初二)
    public function lunarToSolar($date,$type = 0)
    {
